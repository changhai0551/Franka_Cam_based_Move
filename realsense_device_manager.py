...
class DeviceManager:
    ...
    def poll_frames(self):
        ...
        for (serial, device) in self._enabled_devices.items():
            ...
            # if not frameset.get_depth_frame():
            #     print 'you are sb'
            #     continue
    ...
    def poll_aligned_frames(self):
        """
        Poll for aligned (color frame) frames from the enabled Intel RealSense devices.
        If temporal post processing is enabled, the depth stream is averaged over a certain amount of frames
        ...
        for (serial, device) in self._enabled_devices.items():
            ...
            if not frameset.get_depth_frame() or not frameset.get_color_frame():
                continue
            frameset = rs.align(rs.stream.color).process(frameset)
            # pdb.set_trace()
            if frameset.size() == 3 or 2:
                frames[serial] = {}
                # pdb.set_trace()
                for stream in streams:
                    if (rs.stream.infrared == stream.stream_type()):
    ...
    def get_aligned_frames(self):
        ...
        for (serial, device) in self._enabled_devices.items():
            ...
            if frameset.size() == 3:
                frames[serial] = {}
                frames[serial]['color'] = frameset.get_color_frame()
                frames[serial]['depth'] = frameset.get_depth_frame()
